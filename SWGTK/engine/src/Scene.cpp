		...
		if(_pimpl->_destroyFunc.has_value())
{
			...
			if(file.valid())
			{
				...
				if(up && cr)
				{
					return new GameScene::Node{
						._createFunc=static_cast<std::function<SSC(GameScene&)>>(*cr),
						._updateFunc=static_cast<std::function<SSC(GameScene&, float)>>(*up),
						._destroyFunc=(dest) ? std::optional<std::function<void(GameScene&)>>{static_cast<std::function<void(GameScene&)>>(*dest)} : std::nullopt
					};
				}
			}
		}
	...
	// void GameScene::InitLua(sol::state& lua)
	// {
	...
	// void GameScene::InitLuaInput(sol::state& lua)
	// {
